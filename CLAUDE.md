...
- **`VectorStore`** (`src/store/mod.rs`) — storage abstraction (`init`, `add_document`, `search`, plus `document_count`/`recent_documents` used by the `status` command). `SqliteVectorStore` (`src/store/sqlite.rs`) is the default: embeddings are little-endian `f32` BLOBs in a `documents`/`chunks` schema; `add_document` is an upsert that deletes prior chunks for the same `source_path` inside one transaction and records the source file's `size` and md5 `hash`; `document_fingerprint` returns them (only for documents whose last add succeeded), letting the `add` command skip an unchanged file before parsing/embedding. There is no schema migration — databases created with an older schema must be recreated; `search` is a brute-force cosine-similarity scan over all chunks in Rust (chunks whose `dim` doesn't match the query are skipped; it errors only if all chunks mismatch). A new backend (Qdrant, pgvector, ...) implements this trait and gets wired in `main.rs`.
- **`DocumentParser`** (`src/parser/mod.rs`) — one implementation per document type. `parser_for(path)` walks the static `PARSERS` registry and returns the first parser whose `supports(path)` matches; a new parser must be added to that list. `PlainTextParser` claims text extensions *and* extensionless files, so order matters if a later parser also wants extensionless files. `PdfParser` (`src/parser/pdf.rs`) claims `.pdf` and extracts text via the `pdf-extract` crate. `DocxParser` (`src/parser/docx.rs`) claims `.docx` (not legacy `.doc`) and pulls `w:t` run text out of `word/document.xml` via `zip` + `quick-xml`. `ExcelParser` (`src/parser/excel.rs`) claims `.xlsx`/`.xlsm`/`.xlsb`/`.xls` via `calamine`, rendering each sheet as tab-separated rows. `XmlParser` (`src/parser/xml.rs`) claims `.xml` and strips markup via `quick-xml`, keeping only text/CDATA content (attributes and tag names are dropped). `CodeParser` (`src/parser/code.rs`) claims source-code extensions (Java, Python, Groovy, shell, batch, C family, web, ...) and delegates `parse` to `PlainTextParser` — it only widens the set of claimed extensions; it does not claim extensionless files.
...
- After each change, print a short one-line commit message describing it (e.g. `fix: handle empty chunks`) in the response so the user sees it. Do not commit unless explicitly asked.
